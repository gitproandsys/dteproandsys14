...
def send_ack_dte(url, data, user, passwd):
    r = requests.post(url + SEND_ACK_URL, data=json.dumps(data), auth=HTTPBasicAuth(user, passwd), headers=HEADERS)
    return json.loads(r.text)
